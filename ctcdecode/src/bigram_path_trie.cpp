...
BigramPathTrie::BigramPathTrie() {
  ...
  log_prob_nb_2_prev = -NUM_FLT_INF;

  log_prob_b_cur = -NUM_FLT_INF;
  log_prob_nb_cur = -NUM_FLT_INF;
  log_prob_nb_2_cur = -NUM_FLT_INF;
  score = -NUM_FLT_INF;
...
BigramPathTrie* BigramPathTrie::get_path_trie(int new_char, int new_timestep, bool reset) {
  ...
  if (child != children_.end()) {
    if (!child->second->exists_) {
      ...
      child->second->log_prob_nb_2_cur = -NUM_FLT_INF;
    }
    return (child->second);
  } else {
    if (has_dictionary_) {
      ...
      if (!found) {
        ...
        BigramPathTrie* new_path = new BigramPathTrie;
...
BigramPathTrie* BigramPathTrie::get_path_vec(std::vector<int>& output, std::vector<int>& timesteps) {
  return get_path_vec(output, timesteps, ROOT_);
}

BigramPathTrie* BigramPathTrie::get_path_vec(std::vector<int>& output,
...
void BigramPathTrie::iterate_to_vec(std::vector<BigramPathTrie*>& output) {
  if (exists_) {
    ...
    log_prob_nb_2_prev = log_prob_nb_2_cur;

    log_prob_b_cur = -NUM_FLT_INF;
    log_prob_nb_cur = -NUM_FLT_INF;
    log_prob_nb_2_cur = -NUM_FLT_INF;

    score = log_sum_exp(log_sum_exp(log_prob_b_prev, log_prob_nb_prev), log_prob_nb_2_prev);
...
void BigramPathTrie::set_dictionary(fst::StdVectorFst* dictionary) {
...
void BigramPathTrie::set_matcher(std::shared_ptr<FSTMATCH> matcher) {
  matcher_ = matcher;
}
